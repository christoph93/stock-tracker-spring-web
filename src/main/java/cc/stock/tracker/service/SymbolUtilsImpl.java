...
public class SymbolUtilsImpl implements SymbolUtils {
	...
	public void createSymbolsFromTransactions() {
		...
		distinctSymbols.forEach(symbolString -> {			
			Symbol symbol = new Symbol(null, Date.from(Instant.now()),
					symbolString);
			
			
//			alphaVantageImpl.getClosginPrices(symbolString)
//			System.out.println(symbol.getClosingPrices().firstKey());
//			System.out.println(symbol.getClosingPrices().lastKey());

		});

		/*
		 * TODO: create method to update oldest 5 symbols
		 * 
		 */
